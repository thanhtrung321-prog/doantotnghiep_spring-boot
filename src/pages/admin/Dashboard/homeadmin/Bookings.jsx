...
import { getImageUrl } from "../../api/apiadmin/serviceoffering"; // Import getImageUrl
...
const Bookings = () => {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        const [salonData, customerData, staffData] = await Promise.all([
          fetchSalons(),
          fetchCustomers(),
          fetchStaffBySalon(salonId || 302),
        ]);
        setSalons(salonData || []);
        setCustomers(customerData || []);
        setStaffList(staffData || []);
        if (salonData.length > 0 && !salonId) {
          setSalonId(salonData[0].id.toString());
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setLoading(true);
      try {
        const [fetchedBookings, fetchedServices, fetchedCategories, salonData] =
          await Promise.all([
    ...
    try {
      ...
      const paymentRequest = {
        user: {
          ...
          fullname:
            staffList.find((s) => s.id === Number(newBooking.staffId))
              ?.fullName || "Nhân viên",
          email:
            staffList.find((s) => s.id === Number(newBooking.staffId))?.email ||
            "",
        },
        booking: {
              ...
              <Pie
                ...
                options={{
                  ...
                  plugins: {
                    ...
                    title: {
                      display: true,
                      text: "Phân bố đặt lịch theo trạng thái",
                      color: "white",
                    },
                  },
                }}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {bookedSlots
                        .filter(
                          (slot) =>
                            !calendarStaffFilter ||
                            slot.staffId === Number(calendarStaffFilter)
                        )
                        .map((slot, index) => (
                          <div
                            key={index}
                          ...
                          >
                            ...
                            {formatDateTime(slot.endTime)} (Nhân viên:{" "}
                            {staffList.find((s) => s.id === slot.staffId)
                              ?.fullName || `ID ${slot.staffId}`}
                            )
                          </div>
                        ))}
                    </div>
              ...
              <div className="space-y-4">
                        ...
                        return (
                          ...
                          >
                            <input
                              type="checkbox"
                              value={service.id}
                              checked={newBooking.serviceIds.includes(
                                service.id
                              )}
                              onChange={(e) => {
                ...
                <div>
                  ...
                  <select
                    value={
                      newBooking.startTime
                        ? formatLocalDateTime(newBooking.startTime)
                        : ""
                    }
                    onChange={(e) => {
        ...
        {showEditStatusModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-gray-800 text-white p-6 rounded-lg shadow-2xl w-full max-w-md overflow-y-auto max-h-[80vh]">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold text-purple-300">
                  Cập nhật trạng thái đặt lịch #{editBookingId}
                </h2>
                <button
        ...
        {showEditBookingModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="edit-booking-modal bg-gray-800 text-white p-6 rounded-lg shadow-2xl w-full max-w-3xl overflow-y-auto max-h-[80vh]">
              <div className="flex justify-between items-center mb-4 sticky top-0 bg-gray-800 z-10">
                <h2 className="text-2xl font-bold text-purple-300">
                  Chỉnh sửa đặt lịch #{editBooking.id}
                </h2>
                <button
              ...
              <div className="space-y-4">
                        ...
                        return (
                          ...
                          >
                            <input
                              type="checkbox"
                              value={service.id}
                              checked={editBooking.serviceIds.includes(
                                service.id
                              )}
                              onChange={(e) => {
                                const id = Number(e.target.value);
                ...
                <div>
                  ...
                  <DatePicker
                    ...
                    onChange={(date) =>
                      setEditBooking({ ...editBooking, startTime: date })
                    }
                    dateFormat="dd/MM/yyyy"
                    locale={vi}
                    className="mt-1 block w-full py-2 px-3 border border-gray-300 bg-gray-700 text-white rounded-md"
                ...
                <div>
                  ...
                  <select
                    value={
                      editBooking.startTime
                        ? formatLocalDateTime(editBooking.startTime)
                        : ""
                    }
                    onChange={(e) => {
                      const selectedSlot = availableSlots.find(
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="mt-2 text-gray-300">
                      ...
                      {customerMap[selectedBooking.customerId] ||
                        `Khách hàng ${selectedBooking.customerId}`}
                    </p>
                    <button
                      onClick={() =>
                        handleViewCustomerDetails(selectedBooking.customerId)
                      }
                    ...
                    <button
                      onClick={() =>
                        setDetailDisplayMode(
                          detailDisplayMode === "slider" ? "grid" : "slider"
                        )
                      }
        ...
        {showServiceDetailModal && selectedService && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="service-detail-modal bg-gray-800 text-white p-6 rounded-lg shadow-2xl w-full max-w-4xl overflow-y-auto max-h-[80vh]">
              <div className="flex justify-between items-center mb-4 sticky top-0 bg-gray-800 z-10">
                <h2 className="text-2xl font-bold text-purple-300">
                  Chi tiết dịch vụ: {selectedService.name.split("|")[0]}
                </h2>
                <button
        ...
        {showDeleteModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="delete-booking-modal bg-gray-800 text-white p-6 rounded-lg shadow-2xl w-full max-w-md">
              ...
              <p className="text-gray-300">
                Bạn có chắc chắn muốn xóa đặt lịch #{selectedBookingId}?
              </p>
              <div className="mt-6 flex justify-end space-x-2">
                <button
                  onClick={() => setShowDeleteModal(false)}
