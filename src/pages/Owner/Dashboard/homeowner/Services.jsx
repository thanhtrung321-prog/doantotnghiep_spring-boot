import React, { useState, useEffect, useRef, lazy, Suspense } from "react";
import { toast, ToastContainer } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import Slider from "react-slick";
import "slick-carousel/slick/slick.css";
import "slick-carousel/slick/slick-theme.css";
import {
  fetchUserData,
  fetchSalonData,
  fetchCategoriesBySalon,
  fetchServicesBySalon,
  createService,
  updateService,
  deleteService,
  getImageUrl,
} from "../api/servicesowner";
import { gsap } from "gsap";
import AOS from "aos";
import "aos/dist/aos.css";

const SalonLoadingScreen = lazy(() => import("../homeowner/Loadingowner"));

const ErrorBoundary = ({ children }) => {
  const [hasError, setHasError] = useState(false);

  useEffect(() => {
    const errorHandler = (error, errorInfo) => {
      console.error("Error caught by ErrorBoundary:", error, errorInfo);
      setHasError(true);
    };
    // Note: React doesn't have a direct componentDidCatch in functional components,
    // but we can simulate it for demonstration. In practice, use a class-based ErrorBoundary.
    return () => {
      // Cleanup if needed
    };
  }, []);

  if (hasError) {
    return (
      <div className="bg-gray-800 rounded-xl p-10 text-center text-white">
        <h3 className="text-xl font-medium mb-2">Đã xảy ra lỗi</h3>
        <p className="text-gray-400">
          Không thể tải nội dung. Vui lòng thử lại sau.
        </p>
      </div>
    );
  }
  return children;
};

const Services = () => {
  const [services, setServices] = useState([]);
  const [categories, setCategories] = useState([]);
  const [selectedCategory, setSelectedCategory] = useState("");
  const [searchTerm, setSearchTerm] = useState("");
  const [isLoading, setIsLoading] = useState(true);
  const [showLoadScreen, setShowLoadScreen] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [currentService, setCurrentService] = useState(null);
  const [showDetails, setShowDetails] = useState(null);
  const [displayMode, setDisplayMode] = useState("slider");
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [selectedServiceId, setSelectedServiceId] = useState(null);
  const [salonId, setSalonId] = useState(null);
  const [salonData, setSalonData] = useState(null);
  const serviceCardsRef = useRef([]);
  const titleRef = useRef(null);

  const [formData, setFormData] = useState({
    name: "",
    description: "",
    price: "",
    duration: "",
    categoryId: "",
    salonId: "",
    steps: [{ name: "", description: "", image: "", preview: null }],
  });

  useEffect(() => {
    AOS.init({ duration: 1200, once: true, easing: "ease-out-cubic" });
  }, []);

  useEffect(() => {
    if (titleRef.current) {
      const chars = titleRef.current.querySelectorAll(".char");
      gsap.to(chars, {
        color: [
          "rgb(34, 211, 238)",
          "rgb(236, 72, 153)",
          "rgb(234, 179, 8)",
          "rgb(255, 255, 255)",
        ],
        duration: 1.5,
        stagger: 0.1,
        ease: "sine.inOut",
        repeat: -1,
        repeatDelay: 0.5,
        yoyo: true,
      });
    }
  }, [salonData]);

  useEffect(() => {
    const timer = setTimeout(() => setShowLoadScreen(false), 1500);
    return () => clearTimeout(timer);
  }, []);

  useEffect(() => {
    const fetchUserAndSalonData = async () => {
      const userId = localStorage.getItem("userId");
      if (!userId) {
        toast.error(
          "Không tìm thấy thông tin người dùng. Vui lòng đăng nhập lại."
        );
        return;
      }

      try {
        const user = await fetchUserData(userId);
        setSalonId(user.salonId);
        const salon = await fetchSalonData(user.salonId);
        setSalonData(salon);
      } catch (error) {
        toast.error(
          error.message || "Không thể tải thông tin người dùng hoặc salon"
        );
        console.error("Error fetching data:", error);
      }
    };
    fetchUserAndSalonData();
  }, []);

  useEffect(() => {
    if (salonId) {
      const loadData = async () => {
        setIsLoading(true);
        try {
          const [categoryData, serviceData] = await Promise.all([
            fetchCategoriesBySalon(salonId),
            fetchServicesBySalon(salonId),
          ]);
          setCategories(categoryData);
          setServices(serviceData);
        } catch (error) {
          toast.error(error.message || "Không thể tải dữ liệu");
        } finally {
          setIsLoading(false);
        }
      };
      loadData();
    }
  }, [salonId]);

  useEffect(() => {
    if (services.length > 0 && serviceCardsRef.current.length > 0) {
      gsap.fromTo(
        serviceCardsRef.current.filter(Boolean),
        { opacity: 0, y: 30 },
        { opacity: 1, y: 0, duration: 0.8, stagger: 0.15, ease: "power3.out" }
      );
    }
  }, [services]);

  const handleInputChange = (e) => {
    const { name, value } = e.target;
    setFormData((prev) => ({ ...prev, [name]: value }));
  };

  const handleStepChange = (index, field, value) => {
    setFormData((prev) => {
      const newSteps = [...prev.steps];
      newSteps[index] = { ...newSteps[index], [field]: value };
      return { ...prev, steps: newSteps };
    });
  };

  const handleImageChange = (index, e) => {
    const file = e.target.files[0];
    if (file) {
      setFormData((prev) => {
        const newSteps = [...prev.steps];
        newSteps[index] = {
          ...newSteps[index],
          image: file.name,
          preview: URL.createObjectURL(file),
        };
        return { ...prev, steps: newSteps };
      });
    }
  };

  const addStep = () => {
    setFormData((prev) => ({
      ...prev,
      steps: [
        ...prev.steps,
        { name: "", description: "", image: "", preview: null },
      ],
    }));
  };

  const removeStep = (index) => {
    if (formData.steps.length === 1) {
      toast.error("Phải có ít nhất một bước!");
      return;
    }
    setFormData((prev) => ({
      ...prev,
      steps: prev.steps.filter((_, i) => i !== index),
    }));
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!salonId) {
      toast.error("Không xác định được salon");
      return;
    }
    try {
      const serviceData = {
        name: formData.steps.map((step) => step.name).join("|"),
        description: formData.steps.map((step) => step.description).join("|"),
        price: parseFloat(formData.price),
        duration: parseInt(formData.duration),
        categoryId: parseInt(formData.categoryId),
        salonId: salonId,
        image: formData.steps.map((step) => step.image).join("|"),
      };

      if (isEditing) {
        const updatedService = await updateService(
          currentService.id,
          serviceData
        );
        setServices((prev) =>
          prev.map((s) => (s.id === currentService.id ? updatedService : s))
        );
        toast.success("Dịch vụ đã được cập nhật thành công!");
      } else {
        const newService = await createService(serviceData);
        setServices((prev) => [...prev, newService]);
        toast.success("Dịch vụ đã được tạo thành công!");
      }
      setShowModal(false);
      resetForm();
    } catch (error) {
      toast.error(error.message || "Không thể lưu dịch vụ");
    }
  };

  const handleEdit = (service) => {
    setIsEditing(true);
    setCurrentService(service);
    const names = service.name.split("|");
    const descriptions = service.description.split("|");
    const images = service.image.split("|");
    const steps = names.map((name, index) => ({
      name,
      description: descriptions[index] || "",
      image: images[index] || "",
      preview: images[index] ? getImageUrl(images[index]) : null,
    }));
    setFormData({
      name: service.name.split("|")[0],
      description: service.description.split("|")[0],
      price: service.price.toString(),
      duration: service.duration.toString(),
      categoryId: service.categoryId.toString(),
      salonId: salonId.toString(),
      steps,
    });
    setShowModal(true);
  };

  const handleDelete = (id) => {
    setSelectedServiceId(id);
    setShowDeleteModal(true);
  };

  const confirmDelete = async () => {
    try {
      await deleteService(selectedServiceId);
      setServices((prev) => prev.filter((s) => s.id !== selectedServiceId));
      toast.success("Dịch vụ đã được xóa thành công!");
    } catch (error) {
      toast.error(error.message || "Không thể xóa dịch vụ");
    } finally {
      setShowDeleteModal(false);
      setSelectedServiceId(null);
    }
  };

  const resetForm = () => {
    setFormData({
      name: "",
      description: "",
      price: "",
      duration: "",
      categoryId: "",
      salonId: salonId ? salonId.toString() : "",
      steps: [{ name: "", description: "", image: "", preview: null }],
    });
    setIsEditing(false);
    setCurrentService(null);
  };

  const filteredServices = services.filter((service) => {
    const matchesCategory =
      !selectedCategory || service.categoryId === parseInt(selectedCategory);
    const matchesSearch = service.name
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    return matchesCategory && matchesSearch;
  });

  const getCategoryName = (categoryId) => {
    const category = categories.find((cat) => cat.id === categoryId);
    return category ? category.name : "Không xác định";
  };

  const getCategoryImage = (categoryId) => {
    const category = categories.find((cat) => cat.id === categoryId);
    return category && category.image
      ? `http://localhost:8086/category-images/${category.image}`
      : "/images/placeholder.png";
  };

  const parseSteps = (service) => {
    const names = service.name.split("|");
    const descriptions = service.description.split("|");
    const images = service.image.split("|");
    return names.map((name, index) => ({
      name,
      description: descriptions[index] || "",
      image: images[index] || "",
    }));
  };

  const sliderSettings = {
    dots: true,
    infinite: true,
    speed: 500,
    slidesToShow: 1,
    slidesToScroll: 1,
    autoplay: true,
    autoplaySpeed: 10000,
    afterChange: (index) => {
      setShowDetails((prev) => ({ ...prev, activeIndex: index }));
    },
  };

  const handleStepNavigation = (currentIndex, direction) => {
    const service = services.find((s) => s.id === showDetails.id);
    const steps = parseSteps(service).slice(1);
    const newIndex = direction === "next" ? currentIndex + 1 : currentIndex - 1;
    if (newIndex >= 0 && newIndex < steps.length) {
      setShowDetails((prev) => ({ ...prev, activeIndex: newIndex }));
    }
  };

  const titleText = salonData
    ? `${salonData.name} - Quản lý dịch vụ`
    : "Salon Quản lý dịch vụ";
  const titleChars = titleText.split("").map((char, index) => (
    <span key={index} className="char">
      {char}
    </span>
  ));

  return (
    <div className="min-h-screen bg-gray-900 text-white">
      <ToastContainer />
      <header className="relative z-10 bg-gradient-to-b from-gray-900 to-transparent pt-8 pb-16">
        <div className="container mx-auto px-4">
          <h1
            ref={titleRef}
            className="text-4xl font-bold text-center mb-2"
            data-aos="zoom-in"
            data-aos-duration="1000"
          >
            {titleChars}
          </h1>
          <p
            className="text-center text-gray-400 mb-8"
            data-aos="fade-up"
            data-aos-delay="200"
            data-aos-duration="800"
          >
            Quản lý dịch vụ salon của bạn một cách phong cách
          </p>
          <div
            className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8"
            data-aos="fade-up"
            data-aos-delay="400"
            data-aos-duration="1000"
          >
            <div
              className="relative w-full md:w-1/3"
              data-aos="slide-right"
              data-aos-delay="600"
            >
              <input
                type="text"
                placeholder="Tìm kiếm dịch vụ..."
                className="w-full p-3 pl-10 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-white placeholder-gray-400"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
              <svg
                className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-400"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"
                />
              </svg>
            </div>
            <div
              className="w-full md:w-1/3"
              data-aos="slide-left"
              data-aos-delay="700"
            >
              <div className="relative">
                <select
                  className="w-full p-3 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-white appearance-none"
                  value={selectedCategory}
                  onChange={(e) => setSelectedCategory(e.target.value)}
                  disabled={!salonId}
                >
                  <option value="" className="bg-gray-800">
                    Tất cả danh mục
                  </option>
                  {categories.map((category) => (
                    <option
                      key={category.id}
                      value={category.id}
                      className="bg-gray-800"
                    >
                      {category.name}
                    </option>
                  ))}
                </select>
                {selectedCategory && (
                  <div className="absolute right-3 top-1/2 transform -translate-y-1/2 flex items-center">
                    <img
                      src={getCategoryImage(parseInt(selectedCategory))}
                      alt={getCategoryName(parseInt(selectedCategory))}
                      className="w-6 h-6 rounded-full object-cover"
                    />
                  </div>
                )}
              </div>
            </div>
            <button
              className="w-full md:w-auto px-6 py-3 bg-orange-500 hover:bg-orange-600 rounded-lg transition-colors duration-300 font-medium"
              onClick={() => {
                resetForm();
                setShowModal(true);
              }}
              data-aos="zoom-in"
              data-aos-delay="800"
            >
              Thêm dịch vụ mới
            </button>
          </div>
        </div>
      </header>
      <main className="container mx-auto px-4 relative z-10 -mt-8">
        <Suspense
          fallback={
            <div className="flex justify-center items-center h-64">
              <div className="w-16 h-16 border-4 border-orange-500 border-t-transparent rounded-full animate-spin"></div>
            </div>
          }
        >
          <ErrorBoundary>
            {showLoadScreen ? (
              <SalonLoadingScreen />
            ) : isLoading ? (
              <div className="flex justify-center items-center h-64">
                <div className="w-16 h-16 border-4 border-orange-500 border-t-transparent rounded-full animate-spin"></div>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredServices.map((service, index) => {
                  const firstStep = parseSteps(service)[0];
                  return (
                    <div
                      key={service.id}
                      ref={(el) => (serviceCardsRef.current[index] = el)}
                      className="bg-gray-800 rounded-xl overflow-hidden shadow-lg hover:shadow-orange-500/20 transform hover:-translate-y-1 transition-all duration-300"
                      data-aos="fade-up"
                      data-aos-delay={index * 150}
                      data-aos-duration="800"
                    >
                      <div className="relative h-48">
                        <img
                          src={getImageUrl(firstStep.image)}
                          alt={firstStep.name}
                          className="w-full h-full object-cover"
                        />
                        <div className="absolute top-0 right-0 bg-orange-500 text-white px-3 py-1 rounded-bl-lg font-medium">
                          {service.price} VND
                        </div>
                        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4">
                          <span className="inline-flex items-center bg-gray-700 text-orange-500 text-xs px-2 py-1 rounded-full">
                            {getCategoryName(service.categoryId)}
                            <img
                              src={getCategoryImage(service.categoryId)}
                              alt={getCategoryName(service.categoryId)}
                              className="w-5 h-5 ml-2 rounded-full object-cover"
                            />
                          </span>
                        </div>
                      </div>
                      <div className="p-5 flex flex-col h-[260px]">
                        <div className="min-h-[120px]">
                          <h3 className="text-xl font-bold mb-2">
                            {firstStep.name}
                          </h3>
                          <p className="text-gray-400 mb-4 line-clamp-3">
                            {firstStep.description}
                          </p>
                        </div>
                        <div className="flex justify-between items-center text-sm text-gray-400 mb-4">
                          <div className="flex items-center">
                            <svg
                              className="w-4 h-4 mr-1 text-orange-500"
                              fill="currentColor"
                              viewBox="0 0 20 20"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <path
                                fillRule="evenodd"
                                d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z"
                                clipRule="evenodd"
                              />
                            </svg>
                            {service.duration} phút
                          </div>
                          <div>ID: {service.id}</div>
                        </div>
                        <div className="flex space-x-2 mt-auto">
                          <button
                            className="flex-1 bg-orange-500 hover:bg-orange-600 text-white py-2 rounded-lg transition-colors duration-300"
                            onClick={() => handleEdit(service)}
                          >
                            Sửa
                          </button>
                          <button
                            className="flex-1 bg-blue-500 hover:bg-blue-600 text-white py-2 rounded-lg transition-colors duration-300"
                            onClick={() =>
                              setShowDetails({ id: service.id, activeIndex: 0 })
                            }
                          >
                            Chi tiết
                          </button>
                          <button
                            className="flex-1 bg-gray-700 hover:bg-gray-600 text-white py-2 rounded-lg transition-colors duration-300"
                            onClick={() => handleDelete(service.id)}
                          >
                            Xóa
                          </button>
                        </div>
                      </div>
                    </div>
                  );
                })}
              </div>
            )}
          </ErrorBoundary>
        </Suspense>
        {!showLoadScreen && !isLoading && filteredServices.length === 0 && (
          <div
            className="bg-gray-800 rounded-xl p-10 text-center"
            data-aos="zoom-in"
            data-aos-duration="1000"
          >
            <svg
              className="w-16 h-16 mx-auto text-gray-500 mb-4"
              fill="currentColor"
              viewBox="0 0 20 20"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fillRule="evenodd"
                d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z"
                clipRule="evenodd"
              />
            </svg>
            <h3 className="text-xl font-medium mb-1">Không tìm thấy dịch vụ</h3>
            <p className="text-gray-400">
              Vui lòng thêm dịch vụ mới hoặc điều chỉnh bộ lọc.
            </p>
          </div>
        )}
      </main>
      {showDetails && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
          <div
            className="bg-gradient-to-b from-gray-900 to-gray-800 rounded-2xl p-10 w-full max-w-[80%] max-h-[80vh] overflow-y-auto shadow-2xl"
            data-aos="zoom-in"
            data-aos-duration="800"
          >
            <button
              className="absolute top-4 right-4 text-cyan-600 hover:text-cyan-500 text-2xl font-bold transition-transform duration-200 hover:scale-110"
              onClick={() => setShowDetails(null)}
            >
              ×
            </button>
            {(() => {
              const service = services.find((s) => s.id === showDetails.id);
              const steps = parseSteps(service);
              const serviceName = steps[0]?.name || "Dịch vụ không xác định";
              return (
                <>
                  <img
                    src={getImageUrl(steps[0]?.image)}
                    alt={serviceName}
                    className="absolute top-4 left-4 w-24 h-24 object-cover rounded-xl border-2 border-cyan-600 shadow-lg"
                  />
                  <h2 className="text-4xl font-extrabold text-center text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-pink-600 mb-8">
                    {serviceName}
                  </h2>
                </>
              );
            })()}
            <div className="flex justify-between items-center mt-32 mb-6">
              <div className="flex space-x-4">
                <button
                  className={`px-6 py-2 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 ${
                    displayMode === "slider"
                      ? "bg-gradient-to-r from-cyan-600 to-pink-600 text-white"
                      : "bg-gray-700 text-gray-300 hover:bg-gray-600"
                  }`}
                  onClick={() => setDisplayMode("slider")}
                >
                  Chế độ Slider
                </button>
                <button
                  className={`px-6 py-2 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 ${
                    displayMode === "steps"
                      ? "bg-gradient-to-r from-cyan-600 to-pink-600 text-white"
                      : "bg-gray-700 text-gray-300 hover:bg-gray-600"
                  }`}
                  onClick={() => setDisplayMode("steps")}
                >
                  Chế độ Từng Bước
                </button>
              </div>
              <div className="text-lg font-semibold text-yellow-400">
                <span className="mr-6">
                  Giá: {services.find((s) => s.id === showDetails.id).price} VND
                </span>
                <span>
                  Thời gian:{" "}
                  {services.find((s) => s.id === showDetails.id).duration} phút
                </span>
              </div>
            </div>
            {(() => {
              const service = services.find((s) => s.id === showDetails.id);
              const steps = parseSteps(service);
              const displaySteps = steps.slice(1);
              if (steps.length <= 1) {
                const step = steps[0];
                return (
                  <div className="flex flex-col items-center">
                    <img
                      src={getImageUrl(step.image)}
                      alt={step.name}
                      className="w-[80%] aspect-square object-cover rounded-2xl border-2 border-cyan-600 shadow-lg mx-auto mb-6"
                    />
                    <div className="text-center">
                      <h3 className="text-2xl font-bold text-cyan-400 mb-3">
                        {step.name || "Không có tên"}
                      </h3>
                      <p className="text-lg text-gray-200 italic">
                        {step.description || "Không có mô tả"}
                      </p>
                    </div>
                  </div>
                );
              } else if (displayMode === "slider") {
                if (displaySteps.length === 1) {
                  const step = displaySteps[0];
                  return (
                    <div className="flex flex-col items-center">
                      <img
                        src={getImageUrl(step.image)}
                        alt={step.name}
                        className="w-[80%] aspect-square object-cover rounded-2xl border-2 border-cyan-600 shadow-lg mx-auto mb-8 transition-transform duration-300 hover:scale-105"
                      />
                      <div className="text-center">
                        <h3 className="text-2xl font-bold text-cyan-400 mb-3">
                          Bước 1: {step.name || "Không có tên"}
                        </h3>
                        <p className="text-lg text-gray-200 italic">
                          {step.description || "Không có mô tả"}
                        </p>
                      </div>
                    </div>
                  );
                } else {
                  return (
                    <div className="flex flex-col items-center">
                      <div className="w-[80%]">
                        <Slider {...sliderSettings}>
                          {displaySteps.map((step, index) => (
                            <div key={index}>
                              <img
                                src={getImageUrl(step.image)}
                                alt={step.name}
                                className="w-full aspect-square object-cover rounded-2xl border-2 border-cyan-600 shadow-lg transition-transform duration-300 hover:scale-105"
                              />
                            </div>
                          ))}
                        </Slider>
                      </div>
                      <div className="mt-12 text-center">
                        <h3 className="text-2xl font-bold text-cyan-400 mb-3">
                          Bước {showDetails.activeIndex + 1}:{" "}
                          {displaySteps[showDetails.activeIndex]?.name ||
                            "Không có tên"}
                        </h3>
                        <p className="text-lg text-gray-200 italic">
                          {displaySteps[showDetails.activeIndex]?.description ||
                            "Không có mô tả"}
                        </p>
                      </div>
                    </div>
                  );
                }
              } else {
                return (
                  <div>
                    {displaySteps.map((step, index) => (
                      <div key={index} className="mb-8 flex flex-col">
                        <div className="flex flex-row items-start">
                          <img
                            src={getImageUrl(step.image)}
                            alt={step.name}
                            className="w-64 h-64 object-cover rounded-xl border-2 border-cyan-600 shadow-lg mr-6 transition-transform duration-300 hover:scale-105"
                          />
                          <div className="min-h-[256px] flex flex-col">
                            <h3 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-600 mb-2">
                              Bước {index + 1}: {step.name || "Không có tên"}
                            </h3>
                            <p className="text-base text-gray-200 italic mt-2">
                              {step.description || "Không có mô tả"}
                            </p>
                          </div>
                        </div>
                        <div className="flex justify-between mt-4">
                          {index > 0 && (
                            <button
                              className="px-4 py-2 bg-gray-700 text-gray-300 hover:bg-gray-600 rounded-lg font-semibold transition-all duration-300"
                              onClick={() =>
                                handleStepNavigation(index, "prev")
                              }
                            >
                              Trước
                            </button>
                          )}
                          <div className="flex-1"></div>
                          {index < displaySteps.length - 1 && (
                            <button
                              className="bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-blue-600 hover:to-cyan-600 text-white font-semibold py-2 px-6 rounded-xl transition-all duration-300 transform hover:scale-105 shadow-md"
                              onClick={() =>
                                handleStepNavigation(index, "next")
                              }
                            >
                              Tiếp
                            </button>
                          )}
                        </div>
                      </div>
                    ))}
                  </div>
                );
              }
            })()}
            <button
              className="mt-8 w-full bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-blue-600 hover:to-cyan-600 text-white font-semibold py-2 px-6 rounded-xl transition-all duration-300 transform hover:scale-105 shadow-md"
              onClick={() => setShowDetails(null)}
            >
              Đóng
            </button>
          </div>
        </div>
      )}
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div
            className="bg-gray-800 p-8 rounded-md shadow-2xl max-w-sm w-full text-white"
            data-aos="zoom-in"
            data-aos-duration="600"
          >
            <h2 className="text-2xl font-semibold mb-4 text-red-600">
              Xác nhận xóa
            </h2>
            <p className="text-lg text-gray-300 mb-6">
              Bạn có chắc muốn xóa dịch vụ này?
            </p>
            <div className="flex justify-between gap-4">
              <button
                className="px-4 py-2 bg-red-600 text-white rounded-full hover:bg-red-700 transition-colors duration-200"
                onClick={() => confirmDelete()}
              >
                Xóa
              </button>
              <button
                className="px-4 py-2 bg-gray-400 text-white rounded-full hover:bg-gray-500 transition-colors duration-200"
                onClick={() => setShowDeleteModal(false)}
              >
                Hủy
              </button>
            </div>
          </div>
        </div>
      )}
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div
            className="bg-gray-800 rounded-xl p-6 w-full max-w-4xl max-h-[80vh] overflow-y-auto"
            data-aos="zoom-in"
            data-aos-duration="800"
          >
            <h2 className="text-2xl font-bold mb-6 text-white">
              {isEditing ? "Sửa dịch vụ" : "Thêm dịch vụ mới"}
            </h2>
            <form
              onSubmit={handleSubmit}
              className="grid grid-cols-1 md:grid-cols-2 gap-6"
            >
              <div>
                <label className="block text-sm font-medium mb-1 text-white">
                  Giá (VND)
                </label>
                <input
                  type="number"
                  name="price"
                  value={formData.price}
                  onChange={handleInputChange}
                  className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 text-white"
                  step="1000"
                  required
                />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1 text-white">
                  Thời gian (phút)
                </label>
                <input
                  type="number"
                  name="duration"
                  value={formData.duration}
                  onChange={handleInputChange}
                  className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 text-white"
                  required
                />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1 text-white">
                  Danh mục
                </label>
                <select
                  name="categoryId"
                  value={formData.categoryId}
                  onChange={handleInputChange}
                  className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 text-white"
                  required
                  disabled={!salonId}
                >
                  <option value="" className="bg-gray-700">
                    Chọn danh mục
                  </option>
                  {categories.map((category) => (
                    <option
                      key={category.id}
                      value={category.id}
                      className="bg-gray-700"
                    >
                      {category.name}
                    </option>
                  ))}
                </select>
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm font-medium mb-1 text-white">
                  Các bước dịch vụ
                </label>
                {formData.steps.map((step, index) => (
                  <div key={index} className="mb-6 p-4 bg-gray-700 rounded-lg">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium mb-1 text-white">
                          {index === 0 ? "Tên dịch vụ" : `Tên bước ${index}`}
                        </label>
                        <input
                          type="text"
                          value={step.name}
                          onChange={(e) =>
                            handleStepChange(index, "name", e.target.value)
                          }
                          className="w-full p-2 bg-gray-600 border border-gray-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 text-white"
                          required
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-medium mb-1 text-white">
                          Hình ảnh
                        </label>
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(e) => handleImageChange(index, e)}
                          className="w-full p-2 bg-gray-600 border border-gray-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 text-white"
                        />
                        {step.preview && (
                          <img
                            src={step.preview}
                            alt="Preview"
                            className="mt-2 w-32 h-32 object-cover rounded-lg border-2 border-cyan-500"
                          />
                        )}
                        {step.image && !step.preview && (
                          <span className="text-gray-400 text-sm mt-1 block">
                            Tên ảnh: {step.image}
                          </span>
                        )}
                      </div>
                      <div className="md:col-span-2">
                        <label className="block text-sm font-medium mb-1 text-white">
                          Mô tả bước
                        </label>
                        <textarea
                          value={step.description}
                          onChange={(e) =>
                            handleStepChange(
                              index,
                              "description",
                              e.target.value
                            )
                          }
                          className="w-full p-2 bg-gray-600 border border-gray-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 text-white"
                          required
                          rows="2"
                        />
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => removeStep(index)}
                      className="mt-4 px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg transition-colors duration-300"
                    >
                      Xóa bước
                    </button>
                  </div>
                ))}
                <button
                  type="button"
                  onClick={addStep}
                  className="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg transition-colors duration-300"
                >
                  Thêm bước
                </button>
              </div>
              <div className="md:col-span-2 flex gap-4 mt-4">
                <button
                  type="submit"
                  className="flex-1 bg-orange-600 hover:bg-orange-700 text-white py-2 rounded-lg transition-colors duration-300"
                  disabled={isLoading}
                >
                  {isEditing ? "Cập nhật" : "Tạo mới"}
                </button>
                <button
                  type="button"
                  className="flex-1 bg-gray-700 hover:bg-gray-600 text-white py-2 rounded-lg transition-colors duration-300"
                  onClick={() => setShowModal(false)}
                >
                  Hủy
                </button>
              </div>
            </form>
          </div>
        </div>
      )}
    </div>
  );
};

export default Services;
